...
/**
 * This is the model class for table "el_pesanan_detail".
 *
 * The followings are the available columns in table 'el_pesanan_detail':
...
class PesananDetail extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_pesanan, id_produk', 'required'),
			array('id_pesanan, id_produk, qty, status, is_deleted', 'numerical', 'integerOnly'=>true),
			array('disc, total', 'numerical'),
			array('no_order', 'length', 'max'=>11),
			array('created_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_pesanan_detail, id_pesanan, no_order, id_produk, qty, disc, total, status, created_date, is_deleted', 'safe', 'on'=>'search'),
		);
	}
